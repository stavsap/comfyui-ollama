...
if "OLLAMA_API_KEY" not in os.environ:
    raise ValueError("OLLAMA_API_KEY manquante dans .env")
...
# Dictionnaire global par session_id
...
class OllamaSaveContext:
    def __init__(self):
        self._base_dir = (
            os.path.dirname(os.path.realpath(__file__)) + os.path.sep + "saved_context"
        )
    ...
    def ollama_save_context(self, filename, context: list[str] = []):
            ...
            "required": {
                ...
                "mirostat": (
                    "INT",
                    {
                        ...
                        "max": 2,
                        "step": 1,
                        "tooltip": "Whether to use Mirostat sampling. Mirostat is an algorithm that actively maintains the quality of generated text within a desired range during text generation. (0 = disabled, 1 = Mirostat 1, 2 = Mirostat 2.0)",
                    },
                ),
                ...
                "mirostat_eta": (
                    "FLOAT",
                    {
                        ...
                        "min": 0,
                        "step": 0.1,
                        "tooltip": "Mirostat's learning rate parameter influences how quickly the algorithm responds to feedback from the generated text.",
                    },
                ),
                ...
                "mirostat_tau": (
                    "FLOAT",
                    {
                        ...
                        "min": 0,
                        "step": 0.1,
                        "tooltip": "Mirostat's target entropy parameter controls the balance between coherence and diversity in the generated text.",
                    },
                ),
                ...
                "repeat_last_n": (
                    "INT",
                    {
                        ...
                        "max": 64,
                        "step": 1,
                        "tooltip": "Sets how far back for the model to look back to prevent repetition. (0 = disabled, -1 = num_ctx)",
                    },
                ),
                ...
                "repeat_penalty": (
                    "FLOAT",
                    {
                        ...
                        "max": 2,
                        "step": 0.05,
                        "tooltip": "Sets how strongly to penalize repetitions. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient.",
                    },
                ),
                ...
                "temperature": (
                    "FLOAT",
                    {
                        ...
                        "max": 10,
                        "step": 0.05,
                        "tooltip": "Increasing the temperature will make the model answer more creatively.",
                    },
                ),
                ...
                "stop": (
                    "STRING",
                    {
                        "default": "",
                        "multiline": False,
                        "tooltip": "When this pattern is encountered the LLM will stop generating text and return.",
                    },
                ),
                ...
                "num_predict": (
                    "INT",
                    {
                        ...
                        "max": 2048,
                        "step": 1,
                        "tooltip": "Maximum number of tokens to predict when generating text. The default -1 means infinite generation.",
                    },
                ),
                ...
                "debug": (
                    ...
                    {
                        "default": False,
                        "tooltip": "For debugging purposes of the custom nodes, no effect on ollama api.",
                    },
                ),
            },
        }
        ...
        return {
            "required": {
                ...
                "model": (
                    (),
                    {
                        "tooltip": "Select a model for inference. This is a list of available models on the Ollama server. If you don't see any, make sure the Ollama server is running on the url and there are models installed."
                    },
                ),
                "keep_alive": (
                    "INT",
                    {
                        ...
                        "max": 120,
                        "step": 1,
                        "tooltip": "Configures how long ollama keeps the model loaded in memory after inference. -1 = keep alive indefinitely, 0 = unload model immediately after inference",
                    },
                ),
        ...
        return {
            "required": {
                ...
                "keep_context": (
                    ...
                    {
                        "default": False,
                        "tooltip": "If enabled, the model will keep the context of the conversation and use it for the next generation. This is useful for multi-turn conversations or tasks that require context.",
                    },
                ),
                "format": (
                    ["text", "json"],
            ...
            "optional": {
                ...
                "context": (
                    ...
                    {
                        "forceInput": False,
                        "tooltip": "Optionally set an existing model context, useful for multi-turn conversations, follow-up questions.",
                    },
                ),
                "meta": (
                    "OLLAMA_META",
...
class OllamaChat:
    ...
    def ollama_chat(
        ...
        ollama_format: Optional[Literal["", "json"] | JsonSchemaValue] = None
